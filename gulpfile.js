...
const selectCssDev = (cb) => {
	return gulp.src('app/stylus/custom-selects.styl')
		.pipe(plumber())
		...
		.pipe(rename('custom-selects.css'))
...
const watcher = () => {
	...
	watch('app/stylus/**/*.*', gulp.series(selectCssDev, cacheClear));
	watch('app/parts/*.html', gulp.series(htmlDev, cacheClear));
	watch('app/*.html', gulp.series(htmlDev, cacheClear));
};

exports.default = gulp.series(gulp.parallel(copyImg, copySupport, copyVideo, copyFonts, libsCss, libsJs, mainJs, mainCssDev,selectCssDev, htmlDev), gulp.parallel(browser, watcher));
